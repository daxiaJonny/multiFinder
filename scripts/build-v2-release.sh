#!/bin/bash
# Build MultiFinder v2 release with ad-hoc signing without overwriting original MultiFinder.zip.
set -euo pipefail
...
cleanup_generated_directories() {
    ...
    set +e
    if [[ -e "$WORK_DIR" ]]; then
        remove_generated_directory "$WORK_DIR" "$BUILD_ROOT/adhoc-release-v2-work"
    fi
    if [[ -d "$LOCK_DIR" ]]; then
        rmdir -- "$LOCK_DIR"
    fi
...
acquire_build_lock() {
    if ! mkdir -- "$LOCK_DIR"; then
        fail "another release build is already using $BUILD_ROOT"
    fi
}
...
WORK_DIR="$BUILD_ROOT/adhoc-release-v2-work"
OUTPUT_DIR="$BUILD_ROOT/release_v2"
LOCK_DIR="$BUILD_ROOT/adhoc-release-v2.lock"
DERIVED_DATA="$WORK_DIR/DerivedData"
APP_PATH="$DERIVED_DATA/Build/Products/Release/MultiFinder.app"
ZIP_PATH="$OUTPUT_DIR/multiFinder_v2.zip"
CHECKSUM_PATH="$OUTPUT_DIR/multiFinder_v2.zip.sha256"

acquire_build_lock
trap cleanup_generated_directories EXIT
reset_generated_directory "$WORK_DIR" "$BUILD_ROOT/adhoc-release-v2-work"
reset_generated_directory "$OUTPUT_DIR" "$BUILD_ROOT/release_v2"

log "Building arm64+x86_64 Release for MultiFinder v2"
xcodebuild \
...
log "Creating multiFinder_v2.zip"
...
(
    cd "$OUTPUT_DIR"
    shasum -a 256 multiFinder_v2.zip > multiFinder_v2.zip.sha256
    shasum -a 256 -c multiFinder_v2.zip.sha256
)
[[ -s "$CHECKSUM_PATH" ]] || fail "checksum was not created: $CHECKSUM_PATH"

echo
echo "Release artifacts (Theme v2):"
echo "  $ZIP_PATH"
echo "  $CHECKSUM_PATH"
echo "Original MultiFinder.zip is preserved untouched in build/release/."
