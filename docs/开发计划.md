...
- **2026-09-22 进展**：已实现 `mfd --new-tab [path]` 和 `newTab=true` URL 参数，
  在焦点窗格新建标签，文件在父目录中选中，保留原标签；`--workspace` 待实现。
- **规模**：小。
...
- **性能**：万级文件目录的加载与排序基准测试；`DirectoryMonitor` 事件风暴节流。
  2026-09-22 已加入万文件加载及自然排序基准、加载取消和排序竞态回归测试；
  虚拟视图支持过滤版本失效及按文件 ID 同步选择，目录刷新复用已有元数据。
